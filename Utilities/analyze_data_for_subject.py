...
def perform_psychometric_analysis(raw_df, output_folder, subject_name, condition_name, repetition):
    ...
    for bg_val in cmp_bkg_vals:
        # Extract data for this background
        bg_data = extract_data_for_background(raw_df, bg_val)
        lightness_vals = bg_data['lightness_vals']
        n_comp_chsn = bg_data['n_comp_chsn']
        n_trials_per_comp_level = bg_data['n_trials']
        
        # Generate fine-grained values for smooth curve
        lightness_vals_fine = np.linspace(lightness_vals.min(), lightness_vals.max(), 500)
        
        # Fit psychometric function
        data_psignifit = np.array([lightness_vals, n_comp_chsn, 
                                 np.ones(len(lightness_vals))*n_trials_per_comp_level]).T
        result = ps.psignifit(data_psignifit, sigmoid='norm', bounds=custom_bounds)
        
        # Get thresholds at different levels
        threshold_50 = result.parameter_estimate['threshold']
        threshold_76 = result.threshold(0.76)[0]
        # threshold_24 = result.threshold(0.24)[0]
        width = result.parameter_estimate['width']
        
        # Store thresholds data
        thresholds_data.append({
            'standard': std_lightness,
            'background': bg_val,
            # 'threshold_24': threshold_24,
            'threshold_50': threshold_50,
            'threshold_76': threshold_76,
            'width': width
        })
        
        # Generate fitted curve
        y_fit = n_trials_per_comp_level*result.proportion_correct(lightness_vals_fine)
        color = plt.cm.viridis(cmp_bkg_vals.index(bg_val) / len(cmp_bkg_vals))

        # Plot with larger markers (size=10)
        plt.plot(lightness_vals, n_comp_chsn, '.', 
                markersize=12, color=color, label=f'$B_c$ {bg_val:.2f}')
        plt.plot(lightness_vals_fine, y_fit, '-', linewidth=2, color=color)
        
        # Mark threshold points on the curve
        #plt.plot(threshold_24, n_trials_per_comp_level*0.24, 'o', 
                #markersize=8, markeredgecolor='black', markerfacecolor=color)
        plt.plot(threshold_50, n_trials_per_comp_level*0.5, 's', 
                markersize=8, markeredgecolor='black', markerfacecolor=color)
        #plt.plot(threshold_76, n_trials_per_comp_level*0.76, '^', 
                #markersize=8, markeredgecolor='black', markerfacecolor=color)
        
        # Add annotations for all thresholds
        #plt.annotate(f'24%: {threshold_24:.3f}', 
                    #(threshold_24, n_trials_per_comp_level*0.24), 
                    #textcoords="offset points", xytext=(10,5), 
                    #ha='left', color='black', fontsize=11)
        #plt.annotate(f'50%: {threshold_50:.3f}', 
                    #(threshold_50, n_trials_per_comp_level*0.5), 
                    #textcoords="offset points", xytext=(10,5), 
                    #ha='left', color='black', fontsize=11)
        #plt.annotate(f'76%: {threshold_76:.3f}', 
                    #(threshold_76, n_trials_per_comp_level*0.76), 
                    #textcoords="offset points", xytext=(10,5), 
                    #ha='left', color='black', fontsize=11)

    plt.legend(loc='upper left', bbox_to_anchor=(0, 1))  # Upper left inside panel
    plt.xlabel('Comparison Lightness', fontsize=14)
    ...
    #plt.title(f'Standard Lightness = {std_lightness}, Standard Background = {std_bkg_val}', fontsize=14)
    plt.title(f'$L_s$ = {std_lightness}, $B_s$ = {std_bkg_val}', fontsize=14)
    plt.tight_layout()
